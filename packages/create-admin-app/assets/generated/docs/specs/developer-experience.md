...
The local command scans secrets before anything is committed, prepares the commit, creates or converges the GitHub repository, stores the repository secret, pushes `main`, requests the guarded Application Deploy workflow, waits for its validation and deployment result, and fast-forwards the lifecycle commit. Type checks, lint, tests, and builds run once in Actions rather than being duplicated locally. The local process never mutates Worker, D1, Access, or routes.

Application CI validates pull requests only. A `main` push starts only Application Deploy. Its single job installs dependencies once and first runs `pnpm check` without production credentials. Only the following mutation step receives the repository secret; it reuses the validated CLI output and performs only the production-specific build before applying append-only D1 migrations, deploying Worker, role groups, path-specific Access applications and policies, injecting the repository token as the Worker `ACCESS_MANAGEMENT_TOKEN` secret, verifying the private gate and public health path, and committing `infra/lifecycle.json` as deployed. A failed or interrupted run is safe to diagnose and re-run.
