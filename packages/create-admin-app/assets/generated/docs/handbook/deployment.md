...
Then run the interactive journey:
...
The Application Deploy workflow validates its Actions/main capability, applies append-only migrations, converges Worker, One-time PIN, Access application and exact email policy, verifies that the public endpoint is Access-protected, and commits the deployed lifecycle state.
...
These commands do not mutate production. Status checks Worker, D1, Access, route, and lifecycle drift. Status JSON omits redirect query strings and fragments and replaces the opaque Cloudflare Access challenge path segment with `[redacted]`. Logs remain in the current terminal and are not uploaded as artifacts.
