...
The local command may update files, run the secret scan before commit, create or converge the GitHub repository, set the repository Actions secret, push `main`, request the Application Deploy workflow, wait for its validation and deployment result, and fast-forward the lifecycle commit. It leaves type checks, lint, tests, and builds to the guarded workflow and does not mutate Cloudflare production resources.

The Application CI workflow validates pull requests only. A `main` push starts only the Application Deploy workflow. Its single job installs dependencies once, runs `pnpm check` without production credentials, and injects `CLOUDFLARE_API_TOKEN` only into the later mutation step after validation succeeds. The mutation step reuses the validated CLI output and performs only the production-specific build before applying append-only migrations, converging Worker, One-time PIN, role groups and path-specific Access applications, copying the same repository token into the Worker `ACCESS_MANAGEMENT_TOKEN` secret binding, verifying the private gate and public health path, and committing the deployed lifecycle state. This avoids duplicate workflows, runners, dependency installs, checks, and CLI builds while keeping deployment gated by a fresh successful check. The Public application bypasses only data-free `/assets/*` build output and the explicit `/public/*` and `/api/public/*` paths, so role-specific HTML and APIs remain protected while a protected page can load its JS and CSS without crossing Access application sessions. Because the Access API rejects an empty group `include`, an empty role group contains one reserved, non-login `example.com` placeholder email. Deploy and the Owner API normalize this rule and exclude it from member lists, role resolution, and status counts.
