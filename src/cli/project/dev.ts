...
export async function startDevelopmentServer(
	...
	const machine = dependencies.machine ?? (() => cliRuntime().machine);
	if (options.open && machine()) {
		throw usageError(
			"interactive_browser_required",
			"machine mode에서는 브라우저를 열 수 없습니다.",
			"TTY에서 pnpm cli dev --interactive --open을 실행하세요.",
		);
	}
		...
		if (database.mode === "remote") {
			if (machine()) {
				throw configurationError(
					...
					"remote D1 개발 연결에는 사람의 Cloudflare Access 로그인이 필요합니다.",
					"pnpm cli dev --interactive",
				);
			}
