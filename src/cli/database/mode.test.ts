...
describe("development database mode", () => {
	...
	it("requires redeployment instead of silently recreating local state after destroy", async () => {
		const deps = dependencies({
			loadLifecycle: vi.fn(async () => ({
				schemaVersion: 1 as const,
		...
		await expect(
			resolveDevelopmentDatabaseMode("auto", deps),
		).rejects.toMatchObject({
			code: "production_destroyed",
			exitCode: 3,
		});
	});
});
