...
const infrastructureLifecycleSchema = z.strictObject({
	...
	production: z.enum(["predeploy", "deployed", "destroyed"]),
});
...
export async function markProductionDestroyed(
	path = projectPaths.infrastructureLifecycle,
): Promise<boolean> {
	const lifecycle = await loadInfrastructureLifecycle(path);
	if (lifecycle.production === "destroyed") return false;
	await writeJson(path, { ...lifecycle, production: "destroyed" });
	return true;
}
