...
async function createLifecycleFile(
...
describe("infrastructure lifecycle", () => {
	...
	it("records destruction and allows a later deployment to restore production", async () => {
		const path = await createLifecycleFile("deployed");
		await expect(markProductionDestroyed(path)).resolves.toBe(true);
		await expect(loadInfrastructureLifecycle(path)).resolves.toEqual({
			schemaVersion: 1,
			production: "destroyed",
		});
		await expect(markProductionDestroyed(path)).resolves.toBe(false);
		await expect(markProductionDeployed(path)).resolves.toBe(true);
		await expect(loadInfrastructureLifecycle(path)).resolves.toEqual({
			schemaVersion: 1,
			production: "deployed",
