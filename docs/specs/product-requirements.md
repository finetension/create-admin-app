...
4. 제품 CLI의 운영 변경 경로를 GitHub Actions로 제한하고 모든 운영 작업을 Actions 이력으로 재현한다.
...
TypeScript CLI가 자동화 구현의 기준이고 GitHub Actions가 제품이 지원하는 운영 mutation 실행 환경이자 감사 경로다. 로컬 프로젝트 CLI는 Git과 GitHub를 통해 Actions 실행을 준비·요청하고 완료를 기다릴 수 있지만 Cloudflare 운영 리소스를 직접 변경하지 않는다. account-wide write token은 로컬 OS credential store에도 있으므로 Actions-only는 token 자체의 보안 경계라고 주장하지 않는다.
...
- Application Deploy: `main` push 또는 수동 dispatch로 실행. pending migration, Worker 배포, OTP·Access 정책 보장, Worker 활성 상태와 비인증 Access 차단 smoke check
...
- 생성 단계는 GitHub 또는 Cloudflare 자격 증명을 요구하지 않는다.
- 인터랙티브 생성은 완료 뒤 배포를 계속할지 묻고, 비인터랙티브 생성은 `--deploy --yes`를 명시한 경우에만 생성된 프로젝트의 `pnpm cli deploy`를 실행한다. 비인터랙티브 생성은 `--emails`를 요구하고 commit할 변경이 있으면 `--message`를 프로젝트 CLI에 전달한다.
...
- token은 저장소 파일이나 로그에 기록하지 않고 계정 단위 OS credential store와 대상 GitHub 저장소의 repository Actions secret에만 등록한다. PR과 fork workflow는 이 secret을 참조하지 않는다.
- Zero Trust organization이 있으면 Application Deploy workflow가 기존 IdP를 유지한 채 OTP identity provider, Access application과 email Allow policy를 멱등하게 보장한다.
- 배포 후 remote D1 개발에 Access 로그인이 필요하면 TTY의 `pnpm cli dev`가 Wrangler 브라우저 로그인을 사용한다. 비인터랙티브 실행은 브라우저를 열지 않고 `access_login_required` 오류와 사용자 handoff 명령을 반환한다.
...
- 배포 전 전체 계획을 출력한다. TTY에서는 한 번 확인받고 비인터랙티브에서는 `--yes`가 있어야 외부 변경을 실행한다.
