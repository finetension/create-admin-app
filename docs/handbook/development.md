...
Remote D1 binding은 첫 연결에 Cloudflare 계정 인증이 필요하다. TTY의 `pnpm cli dev`는 Wrangler의 브라우저 로그인을 허용한다. machine mode는 브라우저를 열지 않고 `access_login_required` 오류와 `pnpm cli dev --interactive` hint를 반환한다. 로컬 Vite Worker는 배포된 Access gateway를 거치지 않고 기본 Owner actor를 사용하며, 명시적 역할 simulation은 local D1에서만 허용한다. 공통 기반은 이 흐름을 위해 별도 service token을 만들지 않는다.
